...
// Get assigned cities for the rep
$stmt = $pdo->prepare("SELECT city_id FROM user_city_access WHERE rep_id = ?");
$stmt->execute([$rep_id]);
$assigned_city_ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

// Fetch visit details and check access
$stmt = $pdo->prepare("SELECT * FROM visits WHERE id = ?");
$stmt->execute([$visit_id]);
$visit = $stmt->fetch();

if (!$visit || $visit['created_by'] != $rep_id) {
...
<div class="row">
    <div class="col-md-6 offset-md-3">
        ...
        <form action="edit_visit.php?id=<?= $visit_id ?>" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" id="lat" value="<?= $visit['latitude'] ?>">
            <input type="hidden" id="lng" value="<?= $visit['longitude'] ?>">
